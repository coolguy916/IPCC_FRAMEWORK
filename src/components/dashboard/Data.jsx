...
const DataPage = () => {
    // LAYER 0: STATE MANAGEMENT
    const [sidebarOpen, setSidebarOpen] = useState(false);
    const [isSidebarCollapsed, setSidebarCollapsed] = useState(false);
    const [selectedMetric, setSelectedMetric] = useState('soil_health');
    const [regenerativeFieldCount, setRegenerativeFieldCount] = useState(1); 
    const [landAreaInput, setLandAreaInput] = useState(1);
    const [historicalSensorData, setHistoricalSensorData] = useState([]);
    const [appConfig, setAppConfig] = useState(null);
    const [loading, setLoading] = useState({ config: true, sensorData: true });
    const [uploading, setUploading] = useState(false);
    
    // LAYER 1: DATA LAYER
    const { getSensorData, insertData, loading: apiLoading } = useApi();
    ...
    useEffect(() => {
        const fetchAppConfig = async () => {
            const simulatedConfig = {
                palette: { primary: '#16a34a', primary_light: '#dcfce7', primary_dark: '#15803d', primary_transparent: 'rgba(22, 163, 74, 0.15)', secondary: '#64748b', secondary_light: '#e2e8f0', secondary_transparent: 'rgba(100, 116, 139, 0.15)', text_primary: '#1e293b', text_secondary: '#475569', border: '#e2e8f0', bg_subtle: '#f8fafc' },
    ...
    useEffect(() => {
        if (!sensorConnected) {
            setLoading(prev => ({ ...prev, sensorData: true }));
            getSensorData({ limit: 30 }).then(data => setHistoricalSensorData(data || [])).catch(error => { console.error("Failed to fetch historical sensor data:", error); setHistoricalSensorData([]); }).finally(() => setLoading(prev => ({ ...prev, sensorData: false })));
        } else { setLoading(prev => ({ ...prev, sensorData: false })); }
    }, [sensorConnected, getSensorData]);
    
    // LAYER 2: EVENT HANDLERS
    const handleCsvUpload = useCallback(async (parsedData) => {
        if (!parsedData || parsedData.length === 0) return false;
        setUploading(true);
        try { await insertData('plant_collection', parsedData); alert('File data has been successfully uploaded!'); return true; }
        catch (error) { console.error('API UploadError:', error); return false; }
        finally { setUploading(false); }
    }, [insertData]);
    
    // LAYER 3: DATA PROCESSING & SIMULATION LOGIC
    const currentSensorData = useMemo(() => (sensorConnected && realtimeSensorData ? [realtimeSensorData] : historicalSensorData), [sensorConnected, realtimeSensorData, historicalSensorData]);
    const financialMetrics = useMemo(() => (realtimeFinancialData && realtimeFinancialData.length > 0 ? { convCost: realtimeFinancialData[0].conventional_cost || 5330, regenCost: realtimeFinancialData[0].regenerative_cost || 4260 } : { convCost: 5330, regenCost: 4260 }), [realtimeFinancialData]);
    const chartOptions = useMemo(() => { if (!appConfig) return {}; const { palette } = appConfig; return { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', align: 'end', labels: { color: palette.text_secondary, boxWidth: 12 } } }, scales: { y: { beginAtZero: true, grid: { color: palette.border }, ticks: { color: palette.text_secondary } }, x: { grid: { display: false }, ticks: { color: palette.text_secondary } } } }; }, [appConfig]);
        ...
        <div className="min-h-screen bg-slate-50 flex">
            ...
            <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
                ...
                <main className="flex-1 p-4 sm:p-6 overflow-auto">
                    {/* Sections: Header, Metrics, Upload, Charts (No changes here) */}
                    {sensorConnected && <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg flex items-center gap-2"><div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div><span className="text-green-700 font-medium">🟢 Live Data Connected - Real-time updates active</span></div>}
                    <div className="relative p-8 rounded-xl shadow-lg overflow-hidden mb-6 bg-cover bg-center" style={{ backgroundImage: `url(${appConfig.siteProfile.imageUrl})` }}>
